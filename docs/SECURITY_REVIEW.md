...
| Settings migration | auth.py / net_utils.py / migration.py | Confirmed data-integrity problem (source deleted when destination existed; failures swallowed). Unified `safe_migrate` never deletes a colliding source. Auth raises `SettingsError` when a leftover legacy file cannot be moved and no destination exists (no silent reset). Network leftover is preserved and ignored with a warning. |
