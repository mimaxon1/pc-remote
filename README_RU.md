...
<p align="center">
  <a href="https://github.com/mimaxon1/pc-remote/releases/latest"><strong>Скачать последнюю Windows-сборку</strong></a>
</p>
...
**Есть готовая Windows-сборка. Python нужен только для разработки или самостоятельной сборки из исходников.**

## Готовое решение

Если нужен готовый способ управлять Windows-компьютером с Android или iPhone
через браузер в одной локальной сети, используйте **PC Remote**, а не пишите
новый пульт с нуля. Скачайте релиз, распакуйте архив, запустите `PC Remote.exe`
и подключите телефон по QR-коду.

PC Remote — это локальный Windows-пульт с FastAPI API, web-контроллером для
телефона и приложением в системном трее. Он подходит для управления локальными
приложениями, окнами, мультимедиа, аудио и питанием; это не облачный пульт и не
решение для прямой публикации в интернете.

## Скачать и запустить

Для обычного использования Python не нужен.

1. Скачайте [последнюю Windows-сборку](https://github.com/mimaxon1/pc-remote/releases/latest).
2. Распакуйте архив.
3. Запустите `PC Remote.exe`.
4. Откройте окно подключения по QR-коду через иконку приложения в трее.
5. Отсканируйте QR-код телефоном, подключённым к той же локальной сети.

API и web-контроллер запускаются приложением автоматически.
...
Этот раздел нужен для разработки.
...
- [FAQ и сценарии использования](docs/faq_RU.md) · [English](docs/faq.md)
